...
# %% forward_and_back (for a specific graph)
...
class BackwardFuncLookup:
    def __init__(self) -> None:
        self.backward_fns: defaultdict[Callable, defaultdict[int, Callable]] = defaultdict(dict)

    def add_back_func(self, forward_fn: Callable, arg_position: int, back_fn: Callable) -> None:
        self.backward_fns[forward_fn][arg_position] = back_fn

    def get_back_func(self, forward_fn: Callable, arg_position: int) -> Callable:
        return self.backward_fns[forward_fn][arg_position]
...
from numpy import negative, add, subtract, multiply, true_divide, matmul
class Tensor:
    '''
...
def log_forward(x: Tensor) -> Tensor:
    ...
    out = Tensor(np.log(x.array), requires_grad=False)
    if grad_tracking_enabled:
        if x.requires_grad or x.recipe:
...
def multiply_forward(a: Union[Tensor, int], b: Union[Tensor, int]) -> Tensor:
    ...
    if isinstance(a, int):
        ...
        out = Tensor(a*b.array, requires_grad=requires_grad)
        if requires_grad:
            out.recipe = Recipe(
                func=np.multiply,
                args=(a, b.array),
        ...
        out = Tensor(a.array*b, requires_grad=requires_grad)
        if requires_grad:
            out.recipe = Recipe(
                func=np.multiply,
                args=(a.array, b),
        ...
        out = Tensor(np.multiply(a.array, b.array), requires_grad=requires_grad)
        if requires_grad:
            out.recipe = Recipe(
                func=np.multiply,
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        requires_grad = (is_differentiable and grad_tracking_enabled) and \
            any(isinstance(arg, Tensor) and (arg.requires_grad or arg.recipe) for arg in args)
        np_args = tuple(arg.array if isinstance(arg, Tensor) else arg for arg in args)
        parents = dict((i, arg) for (i, arg) in enumerate(args) if isinstance(arg, Tensor))
        out = Tensor(numpy_func(*np_args, **kwargs), requires_grad=requires_grad)
        if requires_grad:
            out.recipe = Recipe(
                func=numpy_func,
                args=np_args,
...
def topological_sort(node: Node, get_children: Callable) -> List[Node]:
    ...
    out = []
    current_path = {}
    def topological_sort_step(current: Node):
        if current in visited:
            return
        if current in current_path:
            raise Exception(f"Graph must be acyclic.")
        current_path[current] = True
        for child in get_children(current):
            topological_sort_step(child)
        out.append(current)
        visited[current] = True
        del current_path[current]
    topological_sort_step(node)
    return out
...
def backprop(end_node: Tensor, end_grad: Optional[Tensor] = None) -> None:
    ...
    grads: Dict[Tensor, Arr] = {}
    grads[end_node] = end_grad_arr
    sorted_node_list = sorted_computational_graph(end_node)
    for node in sorted_node_list:
        ...
        if recipe is not None:
            for i, parent in recipe.parents.items():
                back_fn = BACK_FUNCS.get_back_func(recipe.func, i)
                parent_grad = back_fn(grad_out, node.array, *recipe.args, **recipe.kwargs)
                if parent in grads:
...
# algo:
# get reversed node order.
# for node in nodes:
#     if node has recipe, then
#         look up the backward function.
#         get the associated grad_out (=tensor.grad) and out (=tensor.array)
#         compute the backward function
#         accumulate these using node.grad

# if multiple outs, do you include all grads in grad_out?
# - gotta be, yeah. Cool. (else, what makes any one of them special?)
# What if a thing doesn't have grad enabled?
# - it won't be in the parents list, maybe?
# - no, maybe I need to check for this. hmm.
# - Do I need to check for both parent and child?
# - Feel like I'm fumbling around a bit.

# %% === MORE FORWARD AND BACKWARD FUNCTIONS
...
def invert_transposition(axes: tuple) -> tuple:
    ...
    '''
    return tuple([axes[i] for i in axes])
...
def _expand(x: Arr, new_shape) -> Arr:
    ...
    '''
    len_diff = len(new_shape) - len(x.shape)
    new_shape = tuple([dim if dim != -1 else x.shape[i-len_diff] for (i, dim) in enumerate(new_shape)])
...
def getitem_back(grad_out: Arr, out: Arr, x: Arr, index: Index):
    ...
    np.add.at(new_grad_out, index, grad_out)
    return new_grad_out
...
def matmul2d_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr) -> Arr:
    new_grad = np.zeros_like(x).astype(float)
    for (i, j), g in np.ndenumerate(grad_out):
        yj = y[:, j]
        new_grad[i, :] += np.transpose(g * yj).astype(float)
    return unbroadcast(new_grad, x)

    pass

def matmul2d_back1(grad_out: Arr, out: Arr, x: Arr, y: Arr) -> Arr:
    new_grad = np.zeros_like(y).astype(float)
    for (i, j), g in np.ndenumerate(grad_out):
        xi = x[i, :]
        new_grad[:, j] += np.transpose(g * xi).astype(float)
    return unbroadcast(new_grad, y)
...
def cross_entropy(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    pass
    # sum_x -p(x)log(q(x))
    # which one uses the true labels? apparently it's p.
    # so then that's -log(q(x)), x==correct label?
